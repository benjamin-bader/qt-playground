...
void PictureDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    QColor bannerTextColor = QColor(BANNER_TEXT_COLOR);
    painter->setPen(bannerTextColor);
...
{
    QVariant variantData = index.model()->data(index, Qt::DecorationRole);
    if (! variantData.canConvert<QPixmap>())
    {
        qFatal("This delegate must be used with QPixmap only");
    }
